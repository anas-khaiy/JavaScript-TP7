...
const upload = multer({
    ...
    limits: {
        ...
        // Note: cette limite s'applique par fichier dans array()
    }
});
...
app.use(express.urlencoded({ extended: true })); // Pour traiter les champs texte classiques
...
app.post('/upload', upload.single('fichier'), (req, res) => {
    ...
    <p><img src="/uploads/${req.file.filename}" style="max-width: 400px; display: block; margin: 20px 0; border-radius: 8px;"></p>
...
app.post('/upload-multiple', upload.array('fichiers', 3), (req, res) => {
    ...
    const fileList = req.files.map(file => {
      ...
      <li style="margin-bottom: 20px;">
        ...
        <img src="/uploads/${file.filename}" style="max-width: 200px; display: block; border-radius: 4px;">
      </li>
    `;
    }).join('');
...
app.post('/upload-with-data', uploadMixed, (req, res) => {
    ...
    if (galerieImages.length > 0) {
        const imagesList = galerieImages.map(img => {
            return `
        <div class="gallery-item" style="background: #f8f9fa; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
          <img src="/uploads/${img.filename}" style="max-width: 150px; display: block; margin-bottom: 5px;">
          <small>${img.originalname} (${img.size} octets)</small>
        </div>
      `;
        }).join('');
        galerieHtml = `
      <h3>Images supplémentaires:</h3>
      <div style="display: flex; flex-wrap: wrap; gap: 20px; margin-bottom: 20px;">${imagesList}</div>
    `;
    }
    ...
    <p><img src="/uploads/${mainImage.filename}" style="max-width: 400px; display: block; border-radius: 8px; margin-bottom: 20px;"></p>
